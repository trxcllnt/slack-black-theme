...
NODE_BIN_PATH="$(dirname $NODE_BIN_PATH)"
SET_SUDO_PATH="PATH=\"$PATH:$NODE_BIN_PATH\""
...
if [[ $APP_VER == app-4* ]]; then
    ...
    sudo sh -c "$SET_SUDO_PATH echo npx: $(which npx)" || exit 1;

    sudo sh -c "$SET_SUDO_PATH npx asar extract $SLACK_RESOURCES_DIR/app.asar $SLACK_RESOURCES_DIR/app.asar.unpacked"
